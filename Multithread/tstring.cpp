...
int main(int argc, char** argv)
{
		...
		else
		{
				...
				//daia[0][zForce]=force;
				float k = (float)force/zForce;

				cout<<"K ="<<k;

				for(int i=0;i<=zForce;i++)
						data[1][i]=k*i;

				k=(float)force/(zForce-nodeCount);
				for(int i=(zForce-nodeCount);i<=0;i++)
						data[1][i+nodeCount]=i*k;
		}
